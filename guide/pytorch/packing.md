...
1.  准备好训练的代码，案例中训练代码在mnist/code下, 名为 **mnist.py**
2.  将pytorch\\\_tool.py 工具放入和训练代码目录同级的目录下，即mnist/目录下
3.  Ready To Pack
...
如果训练代码依赖特殊的软件包，例如nltk 等，可以通过Docker命令将软件包放至Docker容器中，然后通过docker
commit将其保存为镜像即可。  
